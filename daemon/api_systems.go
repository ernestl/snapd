...
var deviceManagerSystemAndGadgetAndEncryptionInfo = func(
	...
	return dm.SystemAndGadgetAndEncryptionInfo(systemLabel, checkAction, useCache)
}
...
func getSystemDetailsWithOptionalFixAction(deviceMgr *devicestate.DeviceManager, wantedSystemLabel string, fixAction *secboot.PreinstallAction) (*client.SystemDetails, error) {
	...
	sys, gadgetInfo, encryptionInfo, err := deviceManagerSystemAndGadgetAndEncryptionInfo(deviceMgr, wantedSystemLabel, fixAction, useCache)
	if err != nil {
		return nil, err
	}
...
func postSystemActionCheckPassphrase(c *Command, systemLabel string, req *systemActionRequest) Response {
	...
	const useCache = true
	deviceMgr := c.d.overlord.DeviceManager()
	_, _, encryptionInfo, err := deviceManagerSystemAndGadgetAndEncryptionInfo(deviceMgr, systemLabel, nil, useCache)
	if err != nil {
		return InternalError(err.Error())
	}
...
func postSystemActionFixEncryptionSupport(c *Command, systemLabel string, req *systemActionRequest) Response {
	...
	// TODO: In the future, snapd should be able to identify actions it is responsible for handling,
